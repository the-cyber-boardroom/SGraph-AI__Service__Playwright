...
| 2026-05-17 PM-late | v0.2.28 | Finalisation pass — M-013/14/15/16 closed in one session | Four open M-IDs closed back-to-back. **M-013** (endpoint count): CLAUDE.md line 96 updated from stale "25 endpoints — 3 health + 5 session + 16 browser + 1 sequence" (Routes__Session was removed in v0.1.24) to accurate "16 direct + admin surface". **M-015** (broken links): 19 → **0** in reality tree — 12 forward refs auto-resolved by M-003 landing the 4 remaining domains; 5 depth-miscalculated `../` paths fixed; 2 in archived docs same-pattern fixed. **M-016** (snapshot + findings): `_snapshots/v0.2.28/` cut against commit `759dfaa`; `findings.md` rewritten with v0.2.25→v0.2.28 deltas and `provision_ec2.py` struck from INC-003 (file removed in v0.2.29). **M-014** (VERIFY markers): agent dispatched in parallel, 7 markers resolved across `playwright-service/`, `agent-mitmproxy/`, `ui/`, `qa/`, `infra/` indexes — full rewrites for the 3 that needed post-BV2.11/BV2.12 path reconciliation. 4 new findings surfaced: `INC-006` (dead test placeholder), `INC-007` (`Routes__Web` accepts all HTTP methods, not GET-only as docs claimed), `M-018` (plugin sub-folder verification spot-flag), `M-019` (`SIDECAR_ATTACH` UI gap). Only `M-017` remains deferred (CLAUDE.md rule for INC-004 — needs Architect sign-off). |
