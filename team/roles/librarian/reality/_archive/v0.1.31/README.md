...
15. [`15__sp-cli-ui-dev-agent-dashboard.md`](15__sp-cli-ui-dev-agent-dashboard.md) — **NEW (2026-04-29).** Full admin dashboard rewrite (dev-agent session, PRs 1–6). `settings-bus.js` singleton with vault persistence; `sp-cli-settings-view` full implementation; 7 plugin cards + `sp-cli-launcher-pane` + `sp-cli-launch-panel` (tab-based launch flow); 7 per-plugin detail panels (`sp-cli-{linux,docker,elastic,vnc,prometheus,opensearch,neko}-detail`) each with shared `_shared/` widgets (stack-header, ssm-command, network-info, stop-button); `sg-remote-browser` embedded in VNC detail; right-column panel visibility wired to settings-bus; layout init deferred to `sp-cli:settings.loaded` to fix vault-read race. Auto-generated stack name `{type_id}-{word}-{4digits}` in launch form. POST endpoint corrected to `/xxx/stack` (singular).
16. [`16__sg-aws-dns-and-acm.md`](16__sg-aws-dns-and-acm.md) — **NEW (2026-05-15).** `sg aws dns` (Route 53 management) + `sg aws acm` (ACM inventory) CLI surface. Brief: `team/humans/dinis_cruz/claude-code-web/05/15/08/architect__sg-aws-dns__plan.md`. Three slices in one branch — **P0** read-only (zones / records list+get, ACM list+show); **P1** record mutations (add / update / delete) + `dig`-based authoritative-NS verification (zero-cache-pollution by default) + smart-verify with verbatim info lines + `Route53__Zone__Resolver` (deepest-zone matching) + `Route53__Instance__Linker` + `dns instance create-record` + `--wait` (poll `GetChange` to INSYNC); **P1.5** opt-in cache-polluting modes (`--public-resolvers` 8-way fan-out, `--local` host dig, `--all`) gated behind verbatim WARNING banners. Plus post-P1.5 ergonomics: smart `records add` positional dispatch (auto-resolve IP from latest / named instance), default zone moved to `sg-compute.sgraph.ai` (env override), CLI restructure to `zone` (singular) + `zones` (plural), new `zone check` (orphaned / stale / unmatched / ignored classification with ACM-validation cross-reference + alias-record support + DKIM / SES / DMARC / SPF / autodiscover pattern detection), new `zone purge` (batch DELETE in one ChangeBatch). 136 unit tests, no mocks. Wired into `sg_compute/cli/Cli__SG.py`.
