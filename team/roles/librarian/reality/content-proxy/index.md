...
- `Cli__Content_Proxy` (`Spec__CLI__Builder`): 8 standard verbs (`list/info/create/delete/wait/health/connect/exec`) + `ami`/`cert` groups + a **`local up|down|status|logs|smoke`** group wrapping `docker compose` on the committed local stack (`smoke` = curl the `/mitm-proxy` chain check through mitmproxy-ext). Registered in `sg_compute/cli/Cli__SG.py` as **`sg content-proxy`** (alias `cp`).
