...
| `vnc/service/Vnc__Service.py` | Tier-1 orchestrator. Read paths: `list_stacks(region)`, `get_stack_info(region, stack_name)`, `delete_stack(region, stack_name)`, `health(region, stack_name, username, password)`, **`flows(region, stack_name, username, password)`** (per N4 — peek mitmweb flows; no auto-export). `health` flips state to READY only when both `nginx_ready` AND `mitmweb_ready` pass; `flow_count = -1` sentinel when mitmweb is unreachable. `setup()` lazy-wires `aws_client` + `probe` + `mapper` + `ip_detector` + `name_gen`. `create_stack` lands in step 7f. Includes `_flow_summary_from_mitmweb` pure mapper (mitmweb `/api/flows` JSON → `Schema__Vnc__Mitm__Flow__Summary`). |
...
114 unit tests — primitives + enums + AWS-client setup() wiring + 4 per-concern AWS helpers + 7 schemas + 2 collections + HTTP base + probe + Caller__IP__Detector + Random__Stack__Name__Generator + Stack__Mapper + Service (read paths + flows + setup() chain). Every AWS- and HTTP-touching class is exercised through real `_Fake_*` subclasses (no mocks).
