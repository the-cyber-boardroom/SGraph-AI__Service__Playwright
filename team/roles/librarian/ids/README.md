...
| `M-016` | Cut next catalogue snapshot — `library/catalogue/_snapshots/v0.2.28/` — and re-run `findings.md` health metrics. Carry forward outstanding INC-004/INC-005 surfacing. | ❌ NOT STARTED | Librarian | next session |
| `M-017` | After regression `bcd33439` (INC-004): propose a CLAUDE.md rule entry forbidding shared-foundation refactors mixed with dependent-slice CLI rewires in a single commit. Wait for Architect confirmation before adding. | ❌ NOT STARTED | Architect + Librarian | next session |
