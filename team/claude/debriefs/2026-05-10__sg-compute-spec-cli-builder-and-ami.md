...
- **Branch `claude/add-spec-documentation-XB9ug` is pushed; user will merge to `dev` before the next session.** Next agent: always start from `dev` (CLAUDE.md rule 29). Do not branch off `claude/add-spec-documentation-XB9ug`. If for some reason the merge hasn't happened yet, flag it to the user as a blocker before doing any new work.
