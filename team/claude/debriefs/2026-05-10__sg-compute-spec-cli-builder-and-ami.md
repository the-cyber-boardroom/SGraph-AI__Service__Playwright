...
- **The branch `claude/add-spec-documentation-XB9ug` is not merged.** The next agent should be **aware of** this branch (the v0.2.8 changes live there) but **must NOT commit onto it** — branch naming is `claude/{description}-{session-id}` (CLAUDE.md rule 30) and each session owns its own branch. Correct flow: `git fetch origin && git checkout -b claude/{new-slice}-{new-session-id} origin/claude/add-spec-documentation-XB9ug` so the new branch starts where this one ended. If the previous branch has already been merged to `dev` by the time you start, branch off `dev` instead.
