# SG/Compute — Ollama Spec Specification
...
Default instance type for Ollama spec: `g4dn.xlarge` (best cost/capability ratio for 13B
...
```
sg-compute node create --spec ollama
    --region        eu-west-2
...
sg-compute node list --spec ollama    [--region]
sg-compute node info    <name>  [--region]
sg-compute node delete  <name>  [--region]
sg-compute node health  <name>  [--region]
sg-compute node models  <name>  [--region]   (calls /api/tags, lists local models)
sg-compute node pull    <name> <model>       (pull additional model onto running instance)
```
