# SG/Compute — Helpers Layer Specification
...
- Takes only plain primitives (str, int, list[int]) — never spec-specific schema objects
- Returns plain Python types or `Type_Safe` schemas from `helpers/`
- Never calls `event_bus` — that is the spec service's responsibility
...
concatenated by the spec's `User_Data__Builder`.
