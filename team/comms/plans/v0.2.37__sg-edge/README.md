...
| 2 — DNS helper | `SG_Edge__DNS__Helper` over `sg aws dns`: write/read/delete `proxies.<parent>` A, upsert/read `_state.<parent>` TXT, read `_sg.<slug>` TXT; in-memory-fake tests | DNS (seam) | **done** |
| 3 — Edge Waker (FastAPI Lambda) | `lambdas/edge_waker/` — `Serverless__Fast_API` app + `Fast_API__Routes` + combined-deps loader; convergent reconciliation; parallel-fork to Vault Waker; HTTP readiness probe; loading page | Lambda/EC2 (seam) | **done** |
...
| 4.5 — `sg edge` CLI + local edge | `cli/Cli__SG_Edge*` (CLI-1) + `local/` (file-backed `sg edge local` setup/register/request/check/teardown/usecases) | none | **done** |
| 5 — wiring + setup | `Setup__*`: IAM roles, CF distribution + origin group + ACM wildcard, Edge Waker deploy via `sg aws lambda`, `proxies.<parent>` zone; deploy-via-pytest | live AWS | next |
| 6 — `sg edge_bench` harness | the doc-05 CLI: primitive/flow/failure scenarios, structured JSON, acceptance verdicts. **LOCAL target done** (`Edge_Bench__Suite` + 8 scenarios over the local edge); aws-bench target + docker OpenResty stack pending Slice 5 | local **done** / AWS pending | **partial** |
