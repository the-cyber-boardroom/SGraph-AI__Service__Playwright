...
#   ec2-state, action, elapsed-ms, request-id).  Every invocation emits exactly
#   one JSON line to stdout — Lambda forwards stdout to CloudWatch Logs.
#
# State paths:
#   UNKNOWN slug        → 404 HTML   (state=not_found, action=returned-404)
#   STOPPED             → start EC2, return 202 warming   (state=started, action=started-ec2)
