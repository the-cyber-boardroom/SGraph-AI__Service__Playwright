...
FUNCTION_VERSION = 'v0.0.3'                                                            # bump .y on every JS change below

# _normalise() strips all whitespace so reformatting doesn't trigger drift.
# Slug__From_Host reads x-vault-viewer-host (our owned, unambiguous signal);
# x-forwarded-host is set for interop with standard proxy-aware tooling.
# event.context.requestId is the CloudFront-assigned request ID — same value
# as X-Amz-Cf-Id, so flow_id correlates with CF access logs automatically.
# x-waker-cf-timestamp uses UTC millis-since-epoch (Date.now()) so the value
# can be parsed directly by osbot_utils Timestamp_Now (int milliseconds).
FUNCTION_CODE = ("""\
...
function handler(event) {
    ...
    //    Set both an owned header (our routing signal) and the standard one
    //    (for proxy-aware tooling / log dumps). Overwriting blocks any
    //    viewer-supplied spoof of these names.
    if (req.headers.host) {
        var viewerHost = req.headers.host.value;
        req.headers['x-vault-viewer-host'] = { value: viewerHost };
    ...
    //    Use the CloudFront request ID when available (no randomness needed
    //    and it correlates with CF access logs). Timestamp is UTC millis
    //    since epoch — same shape as osbot_utils Timestamp_Now().
    var flowId = (event.context && event.context.requestId)
        ? event.context.requestId
        : Date.now().toString(36) + '-' + Math.floor(Math.random() * 1e9).toString(36);

    req.headers['x-waker-flow-id']      = { value: flowId };
    req.headers['x-waker-cf-timestamp'] = { value: String(Date.now()) };
