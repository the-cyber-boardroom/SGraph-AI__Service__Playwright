...
const STATIC = { spec_id: 'elastic', type_id: 'elastic', display_name: 'Elastic + Kibana', icon: '🔍', stability: 'stable', boot: '~90s', soon: false, create_endpoint_path: '/elastic/stack' }

class SgComputeElasticCard extends SgComponent {
    static jsUrl = import.meta.url
    get resourceName()   { return 'sg-compute-elastic-card' }
    ...
    _launch() {
        ...
        document.dispatchEvent(new CustomEvent('sp-cli:spec:elastic.launch-requested',   { detail, bubbles: true, composed: true }))
        document.dispatchEvent(new CustomEvent('sp-cli:plugin:elastic.launch-requested', { detail, bubbles: true, composed: true })) // DEPRECATED
    }
}
customElements.define('sg-compute-elastic-card', SgComputeElasticCard)
