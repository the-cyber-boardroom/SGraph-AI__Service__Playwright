# ═══════════════════════════════════════════════════════════════════════════════
# SG/Compute Specs — Podman: tests for Podman__Tags__Builder
# ═══════════════════════════════════════════════════════════════════════════════
...
class test_Podman__Tags__Builder(TestCase):

    def setUp(self):
        self.builder = Podman__Tags__Builder()

    def test_build__name_tag_carries_podman_prefix(self):
        tags    = self.builder.build('fast-fermi', '1.2.3.4', 'tester@example.com')
        as_dict = {t['Key']: t['Value'] for t in tags}
        assert as_dict['Name'] == 'podman-fast-fermi'

    def test_build__name_tag_does_not_double_prefix(self):
        tags    = self.builder.build('podman-fast-fermi', '1.2.3.4')
        as_dict = {t['Key']: t['Value'] for t in tags}
        assert as_dict['Name'] == 'podman-fast-fermi'
    ...
    def test_build__purpose_and_section_are_podman(self):
        tags    = self.builder.build('x-foo', '1.2.3.4', '')
        as_dict = {t['Key']: t['Value'] for t in tags}
        assert as_dict[TAG_PURPOSE_KEY] == 'podman'
