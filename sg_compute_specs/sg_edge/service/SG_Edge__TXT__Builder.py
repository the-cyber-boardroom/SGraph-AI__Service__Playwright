# ═══════════════════════════════════════════════════════════════════════════════
# SG/Compute Specs — sg_edge: SG_Edge__TXT__Builder
...
class SG_Edge__TXT__Builder(Type_Safe):
    ...
    def build(self, record : Schema__SG_Edge__TXT__Record) -> str:             # schema -> wire string
    ...
    def parse(self, txt : str) -> Schema__SG_Edge__TXT__Record:                # wire string -> schema; raises on invalid
        ...
        record = Schema__SG_Edge__TXT__Record(version  = Safe_Int__SG_Edge__TXT_Version(version)     ,
                                              ip       = Safe_Str__IP__Address(fields['ip'])         ,
                                              port     = Safe_Int__Port(int(fields['port']))         ,
                                              type     = Enum__SG_Edge__Backend__Type(fields['type']),
                                              launched = Safe_Int__SG_Edge__Unix_Ts(int(fields['launched'])))
        if fields.get('instance'):                                             # optional
            record.instance = Safe_Str__Instance__Id(fields['instance'])
        return record

    def try_parse(self, txt : str) -> Optional[Schema__SG_Edge__TXT__Record]:  # hot-path variant — None instead of raising
        try:
