...
| [`v0.2.64__playwright-test-pages-and-workflows/`](v0.2.64__playwright-test-pages-and-workflows/) | v0.2.64 | PROPOSED | Rebuild the `GET /` "Try it out" test page into a capability-driven console exposing the full 21-endpoint / 24-verb surface; portable-JSON workflows matching `/sequence/execute`; in-app docs from `/health/capabilities`; proposed JS SDK + `window.*` API (open question: precedent repo); no-mocks integration tests + Docker-build job. 7-phase multi-agent plan. Design only — no `INDEX_HTML`/runtime changes. |
