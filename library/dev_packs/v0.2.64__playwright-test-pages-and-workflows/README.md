...
| 05 | [`05__js-api.md`](05__js-api.md) | **(f)** PROPOSED JS SDK + `window.*` console API. **OPEN QUESTION: precedent repo unavailable this session.** |
...
| 8 | **The JS API is PROPOSED and gated on an open question** (precedent repo). The console's `window.*` programmable surface and an optional standalone SDK file are designed here but **not committed to an API shape** until the operator points at the "how we did it in our other tools" precedent. | The cross-repo precedent is not available in this session (see Open Questions). Designing the shape blind would risk divergence from the house style. |
...
|---|----------|----------------|---------------------|
| **Q1** | **JS-API precedent (BLOCKING for brief 05's final shape).** The brief references "how we did the JS API in our other tools." That precedent repo is **NOT available in this session.** Which repo / file is the canonical JS-client + `window.*` pattern to mirror? | Brief 05 designs a proposal; without the precedent it cannot lock the method names, namespacing (`window.sgPlaywright` vs `window.SGP`), promise vs callback style, or whether the SDK ships as a separate served `.js` asset. | Operator points at the precedent repo; until then 05 ships as a *shape proposal* with a "ALIGN-ON-PRECEDENT" block. The only in-repo JS-client found is the admin-site `sgraph_ai_service_playwright__api_site/shared/api-client.js` (a `class ApiClient` with `get/post/delete`) — NOT a public SDK. See 05. |
...
8. A **proposed JS API** is documented and, once Q1 is answered, implementable
...
- [ ] **Q1 (JS-API precedent repo) answered** — gates brief 05 implementation only; phases 1-4 + 6 proceed without it
