...
| 15 | **No vault integration in this milestone.** Slices E (Bedrock), G (scoped creds), and H (observability) all originally proposed writing to `$SG_VAULT_ROOT/...`. For v0.2.29 every artefact that would have been vault-grounded lands instead under `~/.sg/aws/<surface>/...` (same shape, local JSON/JSONL files, same `0600` perms as the v0.2.28 credentials store). Vault re-introduction is a v0.3.x decision once the substrate proves itself locally. | Per Dinis 2026-05-17 — keep Phase 1 vault-free; the local shape is easier to iterate on and we can re-target the writer interface later without breaking the CLI contract. |
| 16 | **Per-slice branch naming: `v0.2.28__{surface}__uNnZY`.** Drop the `claude/aws-primitives-support-` prefix on the per-slice branches and version-stamp them with the **current code version** (`v0.2.28` at the time of this milestone) plus the surface name plus this session's branch id (`uNnZY`). Foundation branch is `v0.2.28__foundation__uNnZY`. Sibling branches: `v0.2.28__s3__uNnZY`, `v0.2.28__ec2__uNnZY`, etc. The integration branch stays `claude/aws-primitives-support-uNnZY` (the original session branch); sibling branches PR into it; it PRs into `dev`. | Per Dinis 2026-05-17 — version-stamping the per-slice branches makes the surface easy to filter (`git branch \| grep v0.2.28__`) and groups related work for the per-milestone review. |
| 17 | **Sonnet self-orchestrates the milestone.** The orchestration plan (`03__sonnet-orchestration-plan.md`) describes a parallel-Sonnet flow with an Opus coordinator. In practice the user switches the Claude Code UI to Sonnet before triggering execution and that single Sonnet drives the sequence — Foundation first, then each sibling pack via sub-agents (or sequentially if subagent fan-out is unavailable). There is no separate Opus coordinator role. | Per Dinis 2026-05-17 — matches actual UI workflow. The "Opus coordinator" in `03__` should be read as "the orchestrating Sonnet session". |
| 18 | **AppSec review happens after the PR is drafted, not before code.** Slices D (IAM graph `delete`) and G (scoped creds) flag AppSec sign-off. The workflow: Sonnet drafts the PR, opens it as a Draft, tags AppSec on the draft PR. Sonnet does not block on a pre-code AppSec sign-off. | Per Dinis 2026-05-17 — keeps the implementation flowing; AppSec gets concrete code to review rather than aspirational briefs. |
| 19 | **Bedrock model-ID alias table** (`library/reference/v0.2.29__bedrock-model-aliases.yaml`) is **drafted by the Slice E Sonnet**; Dinis signs off. The Sonnet picks the canonical model for each `(provider, region)` tuple from `boto3 bedrock list-foundation-models` against the user's account; the YAML is a deliverable of the Slice E PR. | Per Dinis 2026-05-17 — Sonnet has the account access; Dinis gates the substantive choices (which Opus, which Nova, etc.). |
| 20 | **Test AWS accounts and credits are already in place** (user runs the CLI from a laptop with test-account credentials and AWS credits). Sibling packs may freely provision/tear down real AWS resources during integration tests. No special account-setup step before Foundation. The only thing Dinis owns is **pre-provisioning the 5 seed scope roles for Slice G** (`sg-scope-iam-read-only`, `sg-scope-lambda-list`, `sg-scope-s3-read-bucket-X`, `sg-scope-route53-read-only`, `sg-scope-cloudtrail-read-only`) — Slice G asks Dinis to create them before Phase 1 acceptance runs. | Per Dinis 2026-05-17 — answers the "default test account" question. The scope-roles are the only piece a human has to provision; Dinis confirmed willingness ("I can create them, just ask for it"). |
