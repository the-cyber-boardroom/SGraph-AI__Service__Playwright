# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — Podman__Instance__Helper
# Instance lookup, termination, SSM command execution, and Podman version
# query for sp podman. Mirrors Linux__Instance__Helper with podman check added.
# ═══════════════════════════════════════════════════════════════════════════════
...
from sgraph_ai_service_playwright__cli.podman.service.Podman__AWS__Client           import TAG_PURPOSE_KEY, TAG_PURPOSE_VALUE, TAG_STACK_NAME_KEY
...
class Podman__Instance__Helper(Type_Safe):
    ...
    def get_podman_version(self, region: str, instance_id: str) -> str:            # Returns 'podman version' output or '' on failure
        try:
