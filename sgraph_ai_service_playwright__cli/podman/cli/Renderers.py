# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — Podman Renderers
# Tier-2A Rich-based renderers for the sp podman typer commands. Pure functions.
...
def _state_colour(state: Enum__Podman__Stack__State) -> str:                        # map state → Rich colour tag
    return {Enum__Podman__Stack__State.RUNNING    : 'green' ,
            Enum__Podman__Stack__State.PENDING    : 'yellow',
            Enum__Podman__Stack__State.STOPPING   : 'yellow',
            Enum__Podman__Stack__State.STOPPED    : 'red'   ,
            Enum__Podman__Stack__State.TERMINATING: 'red'   ,
            Enum__Podman__Stack__State.TERMINATED : 'red'   ,
            Enum__Podman__Stack__State.UNKNOWN    : 'white' }.get(state, 'white')
...
def render_info(info: Schema__Podman__Info, c: Console) -> None:
    colour = _state_colour(info.state)
    c.print()
    c.print(Panel(f'[bold]ℹ️   Podman stack[/]  ·  {info.stack_name}  '
    ...
    t.add_row('podman-version', str(info.podman_version)   or '—')
    t.add_row('uptime'        , f'{info.uptime_seconds}s'  if info.uptime_seconds else '—')
    c.print(t)
    c.print()


def render_create(resp: Schema__Podman__Create__Response, c: Console) -> None:
    info = resp.stack_info
    c.print()
    c.print(Panel(f'[bold green]🚀  Created sp podman stack[/]  ·  {info.stack_name}', border_style='green', expand=False))
    c.print()
    ...
    c.print()
    c.print(f'  [dim]Connect:    sp podman connect {info.stack_name} --region {info.region}[/]')
    c.print(f'  [dim]Wait ready: sp podman wait {info.stack_name} --region {info.region}[/]')
    c.print()


def render_health(h: Schema__Podman__Health__Response, c: Console) -> None:
    ...
    t.add_row('podman-ok'     , 'yes' if h.podman_ok else 'no')
    t.add_row('podman-version', str(h.podman_version) or '—')
