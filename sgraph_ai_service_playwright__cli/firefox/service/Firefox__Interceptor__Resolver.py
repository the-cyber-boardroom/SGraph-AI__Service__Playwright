# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — Firefox__Interceptor__Resolver
# Turns a Schema__Firefox__Interceptor__Choice into the two artefacts the
# user-data needs:
#   - Python source written to /opt/sg-firefox/interceptors/active.py
#   - a label for the response / tag
#
# Three shapes:
#   kind=NONE                 → no-op source + label=''
#   kind=NAME   + name='ex'   → baked example source + label=name
#   kind=INLINE + inline_src  → operator source verbatim + label='inline'
# ═══════════════════════════════════════════════════════════════════════════════
...
def list_examples() -> list:                                                        # For `sp firefox interceptors` command
    return sorted(EXAMPLES.keys())


class Firefox__Interceptor__Resolver(Type_Safe):

    def resolve(self, choice: Schema__Firefox__Interceptor__Choice = None) -> tuple:  # → (source: str, label: str)
