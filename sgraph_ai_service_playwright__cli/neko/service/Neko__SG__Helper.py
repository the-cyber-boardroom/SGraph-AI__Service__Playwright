# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — Neko__SG__Helper
# Per-stack security group for Neko (WebRTC browser). Two ingress rules:
#   - Port 443 TCP  : Caddy TLS → Neko HTTP UI
#   - Ports 52000-52100 UDP : WebRTC media stream (operator browser ↔ EC2)
#
# The WebRTC UDP range is what makes Neko fundamentally different from VNC.
...
from sgraph_ai_service_playwright__cli.neko.service.Neko__AWS__Client               import TAG_PURPOSE_KEY, TAG_PURPOSE_VALUE, NEKO_NAMING


VIEWER_PORT          = 443                                                          # Caddy TLS → Neko HTTP UI
WEBRTC_UDP_FROM      = 52000                                                        # WebRTC media: must match NEKO_EPR env var
WEBRTC_UDP_TO        = 52100
