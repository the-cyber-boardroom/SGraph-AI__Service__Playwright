...
# the four inline policies defined in SP__CLI__Lambda__Policy. Uses osbot-aws's
...
class SP__CLI__Lambda__Role(Type_Safe):
    ...
    def ensure(self) -> dict:                                                       # Idempotent — call on every deploy
        ...
        if role.not_exists():                                                       # First-time provision — create with the Lambda trust policy
            role.create(assume_role_policy_document=json_dumps(policy.assume_role_document()))
