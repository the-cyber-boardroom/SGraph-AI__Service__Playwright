...
# ── custom TyperGroup that injects the Lambda Click group ─────────────────────

class _AwsGroup(TyperGroup):
    """TyperGroup subclass for `sg aws` — adds Lambda__App__Group as 'lambda'."""
...
# ── Typer app for aws section ─────────────────────────────────────────────────

app = typer.Typer(
    name            = 'aws',
    help            = 'AWS resource management (DNS, ACM, billing, CloudFront, Lambda, …).',
