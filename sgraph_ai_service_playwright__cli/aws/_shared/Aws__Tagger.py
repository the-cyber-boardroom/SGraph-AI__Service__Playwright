...
class Aws__Tagger(Type_Safe):
    ...
    def tags_for(self, surface: Enum__AWS__Surface, verb: str, session_id: str = '') -> List__Schema__AWS__Tag:
        items = [
        ...
        if session_id:
            items.append(self._tag('sg:session-id', session_id))
        return List__Schema__AWS__Tag(items=items)

    def as_boto3_tags(self, surface: Enum__AWS__Surface, verb: str, session_id: str = '') -> list:
        return [{'Key': str(t.key), 'Value': str(t.value)} for t in self.tags_for(surface, verb, session_id).items]
