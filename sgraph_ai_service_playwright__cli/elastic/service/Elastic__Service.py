...
class Elastic__Service(Type_Safe):                                                  # Pure-logic orchestrator
    ...
    def create_ami_from_stack(self, stack_name : Safe_Str__Elastic__Stack__Name ,
        ...
        # Default AMI name: "sg-elastic-ami-<stack>-<unix-ts>" so re-running yields a unique name (AWS rejects duplicates)
        if not ami_name:
            ami_name = f'sg-elastic-ami-{str(stack_name)}-{int(time.time())}'
        ami_id = self.aws_client.create_ami_from_instance(region       = str(resolved)        ,
                                                           instance_id  = str(info.instance_id),
                                                           stack_name   = stack_name           ,
                                                           ami_name     = ami_name             ,
                                                           description  = f'Auto-generated by sp el ami create from {stack_name}',
                                                           no_reboot    = no_reboot            )
        if not ami_id:
