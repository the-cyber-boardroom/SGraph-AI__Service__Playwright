...
class Schema__Events__Load__Request(Type_Safe):
    ...
    from_consolidated: bool                          = False                        # When set, read a pre-built events.ndjson.gz from the compat-region (decision #8)
    date_iso         : Safe_Str__Text                                               # Required when from_consolidated=True; "YYYY-MM-DD" to load
    compat_region    : Safe_Str__Text                = Safe_Str__Text(DEFAULT_COMPAT_REGION)  # compat-region for consolidated artefacts
