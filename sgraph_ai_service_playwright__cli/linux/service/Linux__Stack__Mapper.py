# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — Linux__Stack__Mapper
# Pure mapper from raw boto3 describe_instances detail dict → Type_Safe
# Schema__Linux__Info. No AWS calls, no network calls.
# ═══════════════════════════════════════════════════════════════════════════════
...
def _state_to_enum(state_str: str) -> Enum__Linux__Stack__State:
    mapping = {'pending'      : Enum__Linux__Stack__State.PENDING    ,
    ...
    return mapping.get(state_str, Enum__Linux__Stack__State.UNKNOWN)


def _uptime_seconds(details: dict) -> int:                                          # Best-effort: 0 when LaunchTime is absent or unparseable
...
class Linux__Stack__Mapper(Type_Safe):

    def to_info(self, details: dict, region: str) -> Schema__Linux__Info:
        return Schema__Linux__Info(
