# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — OpenSearch__Tags__Builder
# Builds the tag list applied to every EC2 instance + SG created for an
# OpenSearch stack. Single responsibility: tag-spec construction. No AWS
...
class OpenSearch__Tags__Builder(Type_Safe):

    def build(self, stack_name: Safe_Str__OS__Stack__Name, caller_ip: Safe_Str__IP__Address, creator: str = '') -> List[dict]:
        return [{'Key': 'Name'                , 'Value': OS_NAMING.aws_name_for_stack(stack_name)},   # Name tag carries 'opensearch-' prefix; never doubled
