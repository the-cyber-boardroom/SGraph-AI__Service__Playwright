# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — Caller__IP__Detector (opensearch-local copy)
# Returns the caller's public IPv4 so OpenSearch__Service can lock the
# per-stack SG ingress to a single /32. Mirrors the elastic helper. Future
# cleanup can promote a shared cli/aws/ version when 3+ sections need it;
# for now each sister section is self-contained.
# ═══════════════════════════════════════════════════════════════════════════════
...
DEFAULT_URL     = 'https://checkip.amazonaws.com'                                   # AWS-hosted, returns plain text (one line + trailing newline), no auth
DEFAULT_TIMEOUT = 10
