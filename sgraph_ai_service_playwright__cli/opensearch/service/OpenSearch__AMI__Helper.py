# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — OpenSearch__AMI__Helper
# AMI lookups for sp os. Single responsibility: read-only AMI queries.
# AMI lifecycle (create / wait / tag / deregister) lives in a separate
...
AMAZON_OWNER       = 'amazon'
TAG_AMI_STATUS_KEY = 'sg:ami-status'                                                # untested | healthy | unhealthy — same convention as sp el
...
class OpenSearch__AMI__Helper(Type_Safe):
    ...
    def latest_healthy_ami_id(self, region: str) -> str:                            # Returns most recent sg:purpose=opensearch + sg:ami-status=healthy AMI; empty if none
        resp   = self.ec2_client(region).describe_images(
            Owners  = ['self']                                                                 ,
