...
- **22 direct endpoints** — 3 health + 6 browser + 2 screenshot + 1 sequence + 1 inspect + 4 session + 1 metrics + 1 index + 1 test-pages + 2 auth (set-cookie). `Routes__Session` was removed in v0.1.24 but is wired again (with `Routes__Inspect`) at `Fast_API__Playwright__Service.py:106-115`; `/test-pages/{name}` landed in the v0.2.64 console effort. Canonical count lives in the reality doc — if this line and the reality doc disagree, the reality doc wins. Admin surface (`/admin/*`) from `Agentic_FastAPI` parent adds ~4 more.
