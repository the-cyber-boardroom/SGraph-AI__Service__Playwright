...
# POST   /images/load          → Schema__Image__Load__Response  (body: {path})
# POST   /images/upload        → Schema__Image__Upload__Response (multipart tar)
# DELETE /images/{name}        → Schema__Image__Remove__Response
# ═══════════════════════════════════════════════════════════════════════════════
...
class Schema__Image__Load__Request(Type_Safe):
    path : str = ''
...
class Routes__Host__Images(Fast_API__Routes):
    ...
    def load_image(self, body: Schema__Image__Load__Request) -> dict:       # POST /images/load
        if not body.path:
    ...
    load_image.__route_path__ = '/load'

    def remove_image(self, name: str) -> dict:                              # DELETE /images/{name}
        result = self._runtime().remove(name)
        if not result.removed:
            raise HTTPException(status_code=404, detail=result.error or f'image {name!r} not found')
        return result.json()
    remove_image.__route_path__ = '/{name}'

    def setup_routes(self):
        self.add_route_get   (self.list_images )
        self.add_route_get   (self.get_image   )
        self.add_route_post  (self.load_image  )
        self.add_route_delete(self.remove_image)
        self.router.add_api_route('/upload', self._upload_image,
                                  methods=['POST'], tags=[self.tag])

    async def _upload_image(self, file: UploadFile = File(...)) -> dict:    # POST /images/upload
        size_bytes = 0
        tmp_path   = ''
        try:
