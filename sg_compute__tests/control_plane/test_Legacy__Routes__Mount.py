...
_TEST_API_KEY = 'test-api-key-legacy-1234567890'                               # ≥ 16 chars; not a real key
...
class test_Legacy__Routes__Mount(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.anon_client  = TestClient(cls.fast_api.app(), raise_server_exceptions=False)
        cls.authed_client = TestClient(cls.fast_api.app(), headers={'X-API-Key': _TEST_API_KEY},
    ...
    def test_api_health_still_reachable(self):
        r = self.authed_client.get('/api/health')
        assert r.status_code == 200
        assert r.json()['status'] == 'ok'

    def test_api_specs_still_reachable(self):
        r = self.authed_client.get('/api/specs')
        assert r.status_code == 200
    ...
    # ── /legacy/* routes enforce outer API-key auth ──────────────────────────

    def test_legacy_catalog_types_is_auth_gated(self):
        r = self.anon_client.get('/legacy/catalog/types')
        assert r.status_code == 401                                            # outer api-key middleware fires

    def test_legacy_docker_stacks_is_auth_gated(self):
        r = self.anon_client.get('/legacy/docker/stacks')
        assert r.status_code == 401

    def test_legacy_ec2_playwright_list_is_auth_gated(self):
        r = self.anon_client.get('/legacy/ec2/playwright/list')
        assert r.status_code == 401

    # ── every legacy response carries X-Deprecated + X-Migration-Path ────────
    # (authenticated request required so it reaches the ASGI wrapper)

    def test_legacy_catalog_has_deprecated_header(self):
        r = self.authed_client.get('/legacy/catalog/types')
    ...
    def test_legacy_docker_has_deprecated_header(self):
        r = self.authed_client.get('/legacy/docker/stacks')
        assert r.headers.get('x-deprecated') == 'true'

    def test_legacy_ec2_has_deprecated_header(self):
        r = self.authed_client.get('/legacy/ec2/playwright/list')
        assert r.headers.get('x-deprecated') == 'true'

    # ── unknown legacy paths ──────────────────────────────────────────────────

    def test_unknown_legacy_path_is_auth_gated(self):
        r = self.anon_client.get('/legacy/nonexistent/path')
        assert r.status_code == 401                                            # outer api-key middleware fires
