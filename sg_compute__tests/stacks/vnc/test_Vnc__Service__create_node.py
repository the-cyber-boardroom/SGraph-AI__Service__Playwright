# ═══════════════════════════════════════════════════════════════════════════════
# sg_compute tests — Vnc__Service.create_node (T2.1)
...
from sg_compute_specs.vnc.service.Caller__IP__Detector                         import Caller__IP__Detector as Vnc__IP__Detector
...
class _FakeIPDetector(Vnc__IP__Detector):
    def detect(self):
...
def _make_service():
    ...
    object.__setattr__(svc, 'user_data_builder'   , Vnc__User_Data__Builder())
    object.__setattr__(svc, 'mapper'              , Vnc__Stack__Mapper()     )
    object.__setattr__(svc, 'ip_detector'         , _FakeIPDetector()        )
    object.__setattr__(svc, 'name_gen'            , Random__Stack__Name__Generator())
    object.__setattr__(svc, 'compose_template'    , Vnc__Compose__Template() )
    object.__setattr__(svc, 'interceptor_resolver', Vnc__Interceptor__Resolver())
    return svc


class test_Vnc__Service__create_node(TestCase):
    ...
    def test_create_node__returns_schema_node_info(self):
        req  = Schema__Node__Create__Request__Base(
            spec_id       = 'vnc'        ,
            node_name     = 'vn-test-001',
            region        = 'eu-west-2'  ,
            instance_type = 't3.large'   ,
            caller_ip     = '1.2.3.4'    ,
        )
        info = self.svc.create_node(req, api_key_ssm_path='/sg-compute/nodes/vn-test-001/sidecar-api-key')
        assert isinstance(info, Schema__Node__Info)

    def test_create_node__spec_id_is_vnc(self):
        req  = Schema__Node__Create__Request__Base(
            spec_id   = 'vnc'        ,
            node_name = 'vn-test-002',
            region    = 'eu-west-2'  ,
        )
        info = self.svc.create_node(req, api_key_ssm_path='/test/path')
        assert info.spec_id == 'vnc'

    def test_create_node__state_is_booting(self):
        req  = Schema__Node__Create__Request__Base(
            spec_id   = 'vnc'        ,
            node_name = 'vn-test-003',
            region    = 'eu-west-2'  ,
        )
        info = self.svc.create_node(req, api_key_ssm_path='/test/path')
    ...
    def test_create_node__ssm_path_stored(self):
        ssm_path = '/sg-compute/nodes/vn-test-005/sidecar-api-key'
        req      = Schema__Node__Create__Request__Base(
            spec_id   = 'vnc'        ,
            node_name = 'vn-test-005',
            region    = 'eu-west-2'  ,
        )
    ...
    def test_create_node__passes_api_key_ssm_path_to_user_data(self):
        rendered = []
        original_render = Vnc__User_Data__Builder.render

        class _CapturingBuilder(Vnc__User_Data__Builder):
            def render(self, stack_name, region, compose_yaml='', interceptor_source='',
                       operator_password='', interceptor_kind='none',
                       registry='', api_key_name='X-API-Key', api_key_ssm_path=''):
                rendered.append({'registry': registry, 'api_key_ssm_path': api_key_ssm_path})
                return original_render(self, stack_name, region,
        ...
        req = Schema__Node__Create__Request__Base(
            spec_id   = 'vnc'        ,
            node_name = 'vn-test-006',
            region    = 'eu-west-2'  ,
        )
        svc.create_node(req, api_key_ssm_path='/test/vnc/ssm/path')
        assert rendered[0]['api_key_ssm_path'] == '/test/vnc/ssm/path'

    def test_create_stack__api_key_ssm_path_threaded_through(self):
        req  = Schema__Vnc__Stack__Create__Request(
            stack_name       = 'vnc-stack-x',
            region           = 'eu-west-2'  ,
            caller_ip        = '1.1.1.1'    ,
        ...
        assert isinstance(resp, Schema__Vnc__Stack__Create__Response)
        assert resp.instance_id == _FAKE_INSTANCE_ID
