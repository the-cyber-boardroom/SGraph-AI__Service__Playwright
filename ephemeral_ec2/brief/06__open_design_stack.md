# Ephemeral EC2 — Open Design Stack Specification
...
ec2 open-design list            [--region]
ec2 open-design info  <name>    [--region]
ec2 open-design delete <name>   [--region]
ec2 open-design health <name>   [--region] [--timeout 300]
ec2 open-design bake-ami <name> [--region]  (future)
```

---

## Two-stack workflow (open-design + Ollama)

```bash
# 1. Launch Ollama on GPU
ec2 ollama create --model llama3.3 --region eu-west-2 --max-hours 4
# → prints private IP: 10.0.1.42

# 2. Launch open-design pointing at Ollama
ec2 open-design create --ollama-ip 10.0.1.42 --max-hours 4 --open
