# Ephemeral EC2 — Stack Contract

## What every stack must provide

A stack is a directory under `ephemeral_ec2/stacks/<name>/` containing:
...
stacks/<name>/
  schemas/
    ...
    Schema__<Name>__Info.py               ← one live instance's state
...
Stack-specific fields are added below these. For open-design that includes `api_key` and
...
def create_stack(self, request: Schema__*__Create__Request, creator: str = '') \
        -> Schema__*__Create__Response:
    # orchestrates: resolve AMI, create SG, render user-data, run instance,
    # emit event, return response

def list_stacks(self, region: str) -> Schema__*__List:
    # DescribeInstances filtered by StackType tag

def get_stack_info(self, region: str, stack_name: str) \
        -> Schema__*__Info | None:

def delete_stack(self, region: str, stack_name: str) \
...
```python
def render(self, stack_name: str, region: str, **stack_kwargs) -> str:
...
Each stack CLI must expose at minimum:

| Command             | Description                                        |
|---------------------|----------------------------------------------------|
| `create`            | Launch a new instance; `--open` opens browser URL  |
| `list`              | Rich table of live instances for the stack type    |
| `info <name>`       | Panel with full details for one instance           |
| `delete <name>`     | Terminate instance and its security group          |
...
| `bake-ami <name>`   | Create AMI from a running instance                 |

---

## Event bus topics

Each stack emits exactly two topics via `event_bus`:

```
<stack-type>:stack.created   → Schema__Stack__Event
<stack-type>:stack.deleted   → Schema__Stack__Event
```

`Schema__Stack__Event` (from `helpers/`) carries:
`type_id`, `stack_name`, `region`, `instance_id`.

---

## Containerisation declaration (per stack)

Each stack service declares:
...
Every stack must have tests in `ephemeral_ec2__tests/stacks/<name>/` that:
...
3. Test `Service.create_stack()` with fake helpers (no AWS calls)
4. Test `Service.delete_stack()` with fake helpers
