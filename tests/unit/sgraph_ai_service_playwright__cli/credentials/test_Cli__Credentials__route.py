# ═══════════════════════════════════════════════════════════════════════════════
# Tests — Cli__Credentials route command
# ═══════════════════════════════════════════════════════════════════════════════
...
class test_Cli__Credentials__route(TestCase):
    ...
    def test_route_matched_rule(self):
        self._store_obj.route_add('aws lambda *', 'lambda-role')
        self._store_obj.role_set(_cfg('lambda-role', 'us-east-1'))
        runner = CliRunner()
        result = runner.invoke(app, ['route', 'aws', 'lambda', 'waker'])
        assert result.exit_code == 0
        assert 'lambda-role' in result.output

    def test_route_no_match_returns_first_role(self):
        self._store_obj.role_set(_cfg('default', 'us-east-1'))
        runner = CliRunner()
        result = runner.invoke(app, ['route', 'aws', 's3', 'ls'])
        assert result.exit_code == 0
        assert 'default' in result.output

    def test_route_no_roles_returns_default_literal(self):
        runner = CliRunner()
        result = runner.invoke(app, ['route', 'aws', 'sts', 'get-caller-identity'])
        assert result.exit_code == 0
        assert 'default' in result.output
