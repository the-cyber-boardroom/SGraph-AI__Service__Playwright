# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — tests for OpenSearch__Tags__Builder
...
class test_OpenSearch__Tags__Builder(TestCase):
    ...
    def test_build__name_tag_carries_opensearch_prefix(self):
        ...
        assert as_dict['Name'] == 'opensearch-quiet-fermi'                            # OS_NAMING.aws_name_for_stack adds prefix when missing

    def test_build__name_tag_does_not_double_prefix(self):                            # Already-prefixed stack name stays unchanged
        tags = self.builder.build('opensearch-prod', '1.2.3.4', 'tester@example.com')
        as_dict = {t['Key']: t['Value'] for t in tags}
        assert as_dict['Name'] == 'opensearch-prod'

    def test_build__includes_full_tag_set(self):
        tags = self.builder.build('os-foo', '1.2.3.4', 'tester@example.com')
        as_dict = {t['Key']: t['Value'] for t in tags}
        assert as_dict[TAG_PURPOSE_KEY]    == TAG_PURPOSE_VALUE
        assert as_dict[TAG_SECTION_KEY]    == TAG_SECTION_VALUE
        assert as_dict[TAG_STACK_NAME_KEY] == 'os-foo'
    ...
    def test_build__creator_falls_back_to_unknown_when_empty(self):
        tags = self.builder.build('os-foo', '1.2.3.4', '')
        as_dict = {t['Key']: t['Value'] for t in tags}
