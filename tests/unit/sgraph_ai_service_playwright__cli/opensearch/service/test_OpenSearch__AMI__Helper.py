# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — tests for OpenSearch__AMI__Helper
# ═══════════════════════════════════════════════════════════════════════════════

from unittest                                                                       import TestCase

from sgraph_ai_service_playwright__cli.opensearch.service.OpenSearch__AMI__Helper   import (AL2023_NAME_FILTER ,
                                                                                              AMAZON_OWNER       ,
                                                                                              OpenSearch__AMI__Helper)
...
class test_OpenSearch__AMI__Helper(TestCase):
    ...
    def test_latest_healthy__returns_most_recent_or_empty(self):
        ...
        kw = self.fake.calls[0][1]
        filters = {f['Name']: f['Values'] for f in kw['Filters']}
        assert filters['tag:sg:purpose']    == ['opensearch']
