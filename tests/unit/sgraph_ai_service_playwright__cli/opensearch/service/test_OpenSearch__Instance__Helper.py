# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — tests for OpenSearch__Instance__Helper
# ═══════════════════════════════════════════════════════════════════════════════

from unittest                                                                       import TestCase

from sgraph_ai_service_playwright__cli.opensearch.service.OpenSearch__AWS__Client   import TAG_PURPOSE_KEY, TAG_PURPOSE_VALUE, TAG_STACK_NAME_KEY
from sgraph_ai_service_playwright__cli.opensearch.service.OpenSearch__Instance__Helper import (INSTANCE_STATES_LIVE      ,
                                                                                                 OpenSearch__Instance__Helper)
...
class test_OpenSearch__Instance__Helper(TestCase):

    def setUp(self):
        self.fake = _Fake_Boto_EC2()
        self.h    = OpenSearch__Instance__Helper()
        self.h.ec2_client = lambda region: self.fake

    def test_list_stacks__filters_by_purpose_and_live_states(self):
        self.fake.instances = [_instance('i-aaa', 'os-prod'),
                               _instance('i-bbb', 'os-dev' )]
    ...
    def test_list_stacks__skips_instances_without_id(self):                          # Defensive: AWS reservations occasionally lack InstanceId in flight
    ...
    def test_find_by_stack_name__hit(self):
        self.fake.instances = [_instance('i-aaa', 'os-prod'),
                               _instance('i-bbb', 'os-dev' )]
        details = self.h.find_by_stack_name('eu-west-2', 'os-dev')
