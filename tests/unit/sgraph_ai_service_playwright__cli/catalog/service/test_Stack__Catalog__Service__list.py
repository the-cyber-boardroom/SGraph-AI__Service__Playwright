...
from tests.unit.sgraph_ai_service_playwright__cli.catalog.service.test_Stack__Catalog__Service import (
    ...
    _Fake_OpenSearch__Service, _Fake_Vnc__Service)
...
def _svc():
    ...
    svc.opensearch_service = _Fake_OpenSearch__Service()
...
class test_Stack__Catalog__Service__catalog(TestCase):
    ...
    def test_get_catalog__available_flags(self):
        ...
        assert entries[Enum__Stack__Type.OPENSEARCH] is True
    ...
    def test_list_all_stacks__unfiltered(self):
        ...
        assert Enum__Stack__Type.OPENSEARCH in types
        assert Enum__Stack__Type.VNC        in types
        assert len(result.stacks) == 4                                              # 1 linux + 1 docker + 1 opensearch + 1 vnc; elastic fake returns empty
