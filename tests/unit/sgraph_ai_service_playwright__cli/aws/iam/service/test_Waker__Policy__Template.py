...
class Test__Waker__Policy__Template:
    ...
    def test_1__builds_policy_with_four_statements(self):                        # logs + ec2 describe + ec2 start + s3 deps-read (ssm:GetParameter removed when Slug__Registry moved to EC2 tags)
        policy = Waker__Policy__Template().build()
        assert len(policy.statements) == 4
