...
class Test__Cli__Fargate__Cluster:
    ...
    def test_1__cluster_list_empty(self):
        client = Fargate__AWS__Client__In_Memory()
        result = runner.invoke(app, ['cluster', 'list'], obj={'fargate_client': client})
        assert result.exit_code == 0
        assert 'No ECS clusters' in result.output

    def test_2__cluster_list_json(self):
        client = _seed_client(Fargate__AWS__Client__In_Memory())
        result = runner.invoke(app, ['cluster', 'list', '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert len(data) == 1
        assert data[0]['cluster_name'] == 'sg-test-cluster'

    def test_3__cluster_describe_existing_json(self):
        client = _seed_client(Fargate__AWS__Client__In_Memory())
        result = runner.invoke(app, ['cluster', 'describe', 'sg-test-cluster', '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert data['cluster_name'] == 'sg-test-cluster'
        assert data['status'] == 'ACTIVE'

    def test_4__cluster_describe_missing(self):
        client = Fargate__AWS__Client__In_Memory()
        result = runner.invoke(app, ['cluster', 'describe', 'no-such-cluster'], obj={'fargate_client': client})
    ...
    def test_6__cluster_create_with_gate(self, monkeypatch):
        ...
        result = runner.invoke(app, ['cluster', 'create', 'new-cluster', '--yes', '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
    ...
    def test_8__cluster_delete_with_gate(self, monkeypatch):
        ...
        monkeypatch.setenv(_MUTATION_ENV, '1')
        result = runner.invoke(app, ['cluster', 'delete', 'sg-test-cluster', '--yes'], obj={'fargate_client': client})
        assert result.exit_code == 0


class Test__Cli__Fargate__TaskDef:

    def test_1__task_def_list_empty(self):
        client = Fargate__AWS__Client__In_Memory()
        result = runner.invoke(app, ['task-def', 'list'], obj={'fargate_client': client})
        assert result.exit_code == 0
        assert 'No task definitions' in result.output

    def test_2__task_def_list_json(self):
        client = _seed_client(Fargate__AWS__Client__In_Memory())
        result = runner.invoke(app, ['task-def', 'list', '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert len(data) == 1
        assert data[0]['family'] == 'hello-world'

    def test_3__task_def_show_json(self):
        client = _seed_client(Fargate__AWS__Client__In_Memory())
        result = runner.invoke(app, ['task-def', 'show', 'hello-world:1', '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
    ...
    def test_5__task_def_register_with_gate(self, monkeypatch):
        ...
        result = runner.invoke(app, ['task-def', 'register',
                                     ...
                                     '--yes', '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
...
class Test__Cli__Fargate__Task:
    ...
    def test_1__task_list_empty(self):
        client = Fargate__AWS__Client__In_Memory()
        result = runner.invoke(app, ['task', 'list'], obj={'fargate_client': client})
        assert result.exit_code == 0
        assert 'No running tasks' in result.output

    def test_2__task_list_json(self):
        client = _seed_client(Fargate__AWS__Client__In_Memory())
        result = runner.invoke(app, ['task', 'list', '--cluster', 'sg-test-cluster', '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert len(data) == 1
        assert 'arn:aws:ecs' in data[0]['task_arn']

    def test_3__task_describe_json(self):
        client = _seed_client(Fargate__AWS__Client__In_Memory())
        task_arns = list(client._tasks.keys())
        arn = task_arns[0]
        result = runner.invoke(app, ['task', 'describe', arn, '--json'], obj={'fargate_client': client})
        assert result.exit_code == 0
        data = json.loads(result.output)
