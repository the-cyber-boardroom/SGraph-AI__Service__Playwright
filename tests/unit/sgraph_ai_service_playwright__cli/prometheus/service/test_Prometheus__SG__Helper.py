# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — tests for Prometheus__SG__Helper
# Real subclass overrides ec2_client(); no mocks. Mirrors the OS SG helper
# tests but expects port 9090 (Prometheus public) instead of 443.
# ═══════════════════════════════════════════════════════════════════════════════

from unittest                                                                       import TestCase

from sgraph_ai_service_playwright__cli.prometheus.service.Prometheus__SG__Helper    import (PROMETHEUS_PORT_EXTERNAL,
                                                                                              Prometheus__SG__Helper )
...
class test_Prometheus__SG__Helper(TestCase):
    ...
    def test_port_constant_is_9090(self):                                           # Prometheus public; no nginx (P1: no UI / no Grafana)
        assert PROMETHEUS_PORT_EXTERNAL == 9090

    def test_ensure_security_group__creates_when_missing(self):
        sg_id = self.sg.ensure_security_group('eu-west-2', 'prom-quiet-fermi', '1.2.3.4')
        ...
        assert create_kw['GroupName']  == 'prom-quiet-fermi-sg'                       # Per PROM_NAMING.sg_name_for_stack
        assert 'prom-quiet-fermi'      in create_kw['Description']
        ...
        assert perm['FromPort'] == PROMETHEUS_PORT_EXTERNAL
        assert perm['ToPort']   == PROMETHEUS_PORT_EXTERNAL
        assert perm['IpRanges'][0]['CidrIp'] == '1.2.3.4/32'

    def test_ensure_security_group__reuses_existing(self):
        self.fake.existing = [{'GroupId': 'sg-existing-prom'}]
        sg_id = self.sg.ensure_security_group('eu-west-2', 'prom-prod', '1.2.3.4')
        assert sg_id == 'sg-existing-prom'
        ops = [c[0] for c in self.fake.calls]
    ...
    def test_ensure_security_group__other_ingress_errors_propagate(self):
        ...
        with self.assertRaises(RuntimeError):
            self.sg.ensure_security_group('eu-west-2', 'prom-prod', '1.2.3.4')
