# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — tests for Prometheus__Stack__Mapper
...
def _details(stack_name='prom-quiet-fermi', allowed_ip='1.2.3.4', state='running', public_ip='5.6.7.8',
             instance_id='i-0123456789abcdef0', sg_id='sg-1234567890abcdef0', name_tag='prometheus-quiet-fermi'):
    return {
...
class test_Prometheus__Stack__Mapper(TestCase):
    ...
    def test_to_info__happy_path(self):
        ...
        assert str(info.stack_name)        == 'prom-quiet-fermi'
        assert str(info.aws_name_tag)      == 'prometheus-quiet-fermi'
        ...
        assert str(info.prometheus_url)    == 'http://5.6.7.8:9090/'                # P1: plain HTTP, no nginx
        assert info.state                  == Enum__Prom__Stack__State.RUNNING

    def test_to_info__no_public_ip_yields_empty_url(self):
        info = self.mapper.to_info(_details(public_ip=''), 'eu-west-2')
        assert str(info.public_ip)      == ''
        assert str(info.prometheus_url) == ''                                       # No URL until AWS assigns the IP
    ...
    def test_to_info__state_mapping(self):                                          # Lifecycle vocabulary lock-in
        for raw, expected in (('pending'      , Enum__Prom__Stack__State.PENDING    ),
