# ═══════════════════════════════════════════════════════════════════════════════
# SP CLI — tests for Prometheus__Instance__Helper
# ═══════════════════════════════════════════════════════════════════════════════
...
from sgraph_ai_service_playwright__cli.prometheus.service.Prometheus__Instance__Helper import (INSTANCE_STATES_LIVE      ,
                                                                                                 Prometheus__Instance__Helper)
...
class test_Prometheus__Instance__Helper(TestCase):

    def setUp(self):
        self.fake = _Fake_Boto_EC2()
        self.h    = Prometheus__Instance__Helper()
        self.h.ec2_client = lambda region: self.fake

    def test_list_stacks__filters_by_purpose_and_live_states(self):
        self.fake.instances = [_instance('i-aaa', 'prom-prod'),
                               _instance('i-bbb', 'prom-dev' )]
    ...
    def test_find_by_stack_name__hit(self):
        self.fake.instances = [_instance('i-aaa', 'prom-prod'),
                               _instance('i-bbb', 'prom-dev' )]
        details = self.h.find_by_stack_name('eu-west-2', 'prom-dev')
