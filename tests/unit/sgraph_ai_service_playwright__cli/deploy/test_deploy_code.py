...
class test_module_surface(TestCase):
    ...
    def test__package_names_cover_every_runtime_dependency(self):                   # Locked — the zip MUST carry the CLI tree, the shared sgraph_ai_service_playwright tree (for boot shim + version file), scripts/ (for provision_ec2 + observability), and agent_mitmproxy/ (for IMAGE_NAME). Missing any one means the agentic Lambda silently falls back to baked-image code for that import path.
