...
class test_module_surface(TestCase):
    ...
    def test__package_names_cover_every_runtime_dependency(self):                   # Locked — the zip MUST carry the CLI tree, sg_compute_specs/ (boot shim + version + mitmproxy + playwright specs), scripts/ (provision_ec2 + observability), and api_site/ (static UI files served at /ui). Missing any one means the agentic Lambda silently falls back to baked-image code for that import path.
        assert PACKAGE_NAMES == ['sgraph_ai_service_playwright__cli'  ,
                                 'sg_compute_specs'                    ,
                                 'scripts'                             ,
                                 'sgraph_ai_service_playwright__api_site']

    def test__version_file_is_shared_with_main_service(self):                       # Single source of truth — version file now at sg_compute_specs/playwright/core/version
        assert VERSION_FILE_RELATIVE == 'sg_compute_specs/playwright/core/version'
