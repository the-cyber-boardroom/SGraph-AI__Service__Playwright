# ═══════════════════════════════════════════════════════════════════════════════
# Tests — Container__Runtime__Docker (subprocess stubbed)
...
class test_Container__Runtime__Docker(TestCase):

    def setUp(self):
        self.runtime = Container__Runtime__Docker()
    ...
    def test_list__empty(self):
        ...
        assert isinstance(result, Schema__Container__List)
        assert result.count == 0
        assert list(result.containers) == []

    def test_list__one_container(self):
        ...
        container = list(result.containers)[0]
        assert isinstance(container, Schema__Container__Info)
        assert container.name   == 'sp-playwright'
        assert container.image  == 'playwright:latest'
    ...
    def test_start__success(self):
        with patch('subprocess.run', return_value=_proc(stdout='abc1234567890\n')):
            result = self.runtime.start(Schema__Container__Start__Request(
                name='mycontainer', image='myimage:latest'))
        assert isinstance(result, Schema__Container__Start__Response)
    ...
    def test_info__found(self):
        ...
        assert isinstance(result, Schema__Container__Info)
    ...
    def test_stop__success(self):
        with patch('subprocess.run', return_value=_proc(rc=0)):
            result = self.runtime.stop('mycontainer')
        assert isinstance(result, Schema__Container__Stop__Response)
        assert result.stopped is True

    def test_remove__success(self):
        with patch('subprocess.run', return_value=_proc(rc=0)):
            result = self.runtime.remove('mycontainer')
